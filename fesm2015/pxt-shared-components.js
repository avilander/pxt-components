import { Directive, ViewContainerRef, Injectable, Injector, Component, Inject, Input, ComponentFactoryResolver, NgModule, ElementRef, Pipe, EventEmitter, Output, ViewChild, NO_ERRORS_SCHEMA, CUSTOM_ELEMENTS_SCHEMA, forwardRef, HostListener, defineInjectable, inject, ChangeDetectorRef } from '@angular/core';
    ...
    canActivate(next, state) {
        ...
        if (token !== 'undefined' && token !== '' && token !== null) {
            try {
                ...
                const decoded = /** @type {?} */ (jwt_decode(token));
                /** @type {?} */
                var tokenAuthorities = localStorage.getItem(pxtAppConfig.systemId + pxtAppConfig.systemPrex + decoded.sub);
                if (tokenAuthorities === 'undefined' || tokenAuthorities === '' || tokenAuthorities === null) {
                    this.authorityService.buscarAuthorities(pxtAppConfig.systemId).subscribe(data => {
                        localStorage.setItem(pxtAppConfig.systemId + pxtAppConfig.systemPrex + decoded.sub, data.authority);
                    });
                }
                else {
                    /** @type {?} */
